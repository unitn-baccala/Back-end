...
const createDish = async (req, res, next) => { 
    if (req.body == null || req.body.jwt_payload == null) {
        res.status(400).send("failed to create dish");
        return;
    }
    ...
    if(ingredients != null && !Array.isArray(ingredients)) {
        res.status(400).send("failed to create dish: invalid 'ingredients' parameter");
        return;
    }
    ...
    if(!every_ingredient_exists) {
        res.status(400).send("failed to create dish: some ingredients do not exist");
        return
    }
    ...
    if(typeof image === 'String') {
        try {
            ...
            res.status(400).send("failed to create dish: failed to convert image from base64");
            return;
        }
    }
